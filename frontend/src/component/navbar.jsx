  ...
  return (
    <nav className='z-40 border bg-white/70 border-gray-200 w-full py-4 sticky top-0 flex justify-around  sm:justify-center gap-12 sm:gap-16 items-center'>
      ...
      <div className=' flex gap-12 items-center '>
          ...
          <ul className='flex justify-center mx-10 items-center gap-8'>
            {
              navbarConfig.map((item, idx) => (
                <Link key={idx} to={`${item.path}`} className='cursor-pointer text-[16px] font-medium'>{item.name}</Link>
               ))
            }
          </ul>
        </div>
        ...
        {showMenu &&
          <div className='absolute top-18 right-0 flex border border-gray-300 text-white w-full bg-gray-700 z-40 '>
            <div className='flex flex-col w-full '>
              {
                navbarConfig.map((item, idx) => (
                  <Link key={idx} to={`${item.path}`} className='px-4 text-center py-3 cursor-pointer  w-full'>{item.name}</Link>
